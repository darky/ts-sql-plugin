...
exports.make_fake_expression = function (type_checker, tags) {
    ...
    var fns = (_a = {},
        ...
        _a[tags.mock] = sql_1.default.mock,
        _a);
    ...
    function fake_expression_from_function_value_declaration(valueDeclaration) {
        var fnBody = (valueDeclaration.body ||
            valueDeclaration.initializer && valueDeclaration.initializer.body);
        var returnStatement = (fnBody && fnBody.statements || []).find(function (s) { return typescript_1.default.isReturnStatement(s); });
        if (returnStatement) {
            var template = returnStatement.getChildren().find(function (c) { return typescript_1.default.isTaggedTemplateExpression(c); });
    ...
    function fake_expression(n) {
        ...
        if (typescript_1.default.isIdentifier(n)) {
            var symbol = type_checker.getSymbolAtLocation(n);
            if (symbol.valueDeclaration) {
                var fromDirectVariable = fake_expression_from_tagged_value_declaration(symbol.valueDeclaration);
                if (fromDirectVariable) {
                    return fromDirectVariable;
                }
            }
            if (symbol.declarations && symbol.declarations.length && typescript_1.default.isAliasSymbolDeclaration(symbol.declarations[0])) {
                var aliasedSymbol = type_checker.getAliasedSymbol(symbol);
        ...
        if (typescript_1.default.isCallExpression(n)) {
            ...
            if (!!fn) {
                ...
                var fake = null;
                if (fn == sql_1.default.mock) {
                    var argument = n.arguments && n.arguments[0];
                    if (argument) {
                        var property = argument.properties.find(function (p) { return p.name.getText() === 'mock'; });
                        var text = (_c = (_b = property) === null || _b === void 0 ? void 0 : _b.initializer) === null || _c === void 0 ? void 0 : _c.text;
                        if (text) {
                            return _a = {}, _a[sql_1.default.symbol] = true, _a.text = text, _a.values = [], _a;
                        }
                    }
                }
            ...
            var symbol = type_checker.getSymbolAtLocation(n.expression);
            if (symbol.valueDeclaration) {
                var resp = fake_expression_from_function_value_declaration(symbol.valueDeclaration);
                if (resp) {
                    return resp;
                }
            }
            if (symbol.declarations && symbol.declarations.length && typescript_1.default.isAliasSymbolDeclaration(symbol.declarations[0])) {
                var aliasedSymbol = type_checker.getAliasedSymbol(symbol);
                if (aliasedSymbol.valueDeclaration) {
                    var resp = fake_expression_from_function_value_declaration(aliasedSymbol.valueDeclaration);
                    if (resp) {
                        return resp;
                    }
                }
