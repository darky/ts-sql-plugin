...
exports.make_fake_expression = function (program, tags) {
    var _a;
    var type_checker = program.getTypeChecker();
    var fns = (_a = {},
    ...
    function fake_expression_from_tagged_value_declaration(valueDeclaration) {
        var childCount = valueDeclaration.getChildCount();
        var template = valueDeclaration.getChildAt(childCount - 1);
    ...
    function fake_expression(n) {
        ...
        if (typescript_1.default.isIdentifier(n)) {
            if (n.kind === typescript_1.default.SyntaxKind.Identifier) {
                var sourceFileStatement = n.getSourceFile().statements.find(function (s) {
                    return s.moduleSpecifier && s.getText().match(n.getText());
                });
                if (sourceFileStatement) {
                    var currentDir = path_1.default.dirname(n.getSourceFile().fileName);
                    var sourceFileRelativePath = sourceFileStatement.moduleSpecifier.getText().replace(/^\'/, '').replace(/\'$/, '');
                    var sourceFilePath_1 = path_1.default.resolve(currentDir, sourceFileRelativePath);
                    var sourceFile = program.getSourceFiles().find(function (f) { return f.fileName.match(sourceFilePath_1); });
                    if (sourceFile) {
                        var map = sourceFile.locals;
                        var symbol = map.get(n.getText());
                        if (symbol) {
                            var valueDeclaration = symbol.getDeclarations()[0];
                            if (valueDeclaration) {
            ...
            if (typeNode && typeNode.kind === typescript_1.default.SyntaxKind.VariableDeclaration) {
                var fromDirectVariable = fake_expression_from_tagged_value_declaration(typeNode.symbol.valueDeclaration);
                if (fromDirectVariable) {
                    return fromDirectVariable;
                }
                var maybeMethodCall_1 = typeNode.getChildAt(typeNode.getChildCount() - 1);
                if (maybeMethodCall_1 && typescript_1.default.isCallExpression(maybeMethodCall_1) && maybeMethodCall_1.getText().match(/this\./)) {
                    var classDeclaration = void 0;
                    while (true) {
                    ...
                    if (typescript_1.default.isClassDeclaration(classDeclaration)) {
                        var methodDeclaration = classDeclaration.members.find(function (m) {
                            return maybeMethodCall_1.expression
                                .name.getText() === m.name.getText();
                        });
                        if (methodDeclaration) {
                            var lastStatementIdx = methodDeclaration.body.statements.length - 1;
                            var lastStatement = methodDeclaration.body.statements[lastStatementIdx];
                            if (lastStatement) {
                                var valueDeclaration = (_c = (_b = lastStatement.flowNode) === null || _b === void 0 ? void 0 : _b.node) === null || _c === void 0 ? void 0 : _c.symbol.valueDeclaration;
                                if (valueDeclaration) {
                                    return fake_expression_from_tagged_value_declaration(valueDeclaration);
                                }
                            }
